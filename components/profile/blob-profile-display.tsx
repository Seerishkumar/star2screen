...
export function BlobProfileDisplay({ userId }: { userId: string }) {
  ...
  const loadProfileData = async () => {
    try {
      ...
      // Load media files with blob URLs
  ...
  const downloadMedia = async (media: MediaFile) => {
    try {
      const response = await fetch(media.blob_url || media.file_url)
      const blob = await response.blob()
      const url = window.URL.createObjectURL(blob)
      const a = document.createElement("a")
      a.href = url
      a.download = `${media.title}.${media.media_type === "image" ? "jpg" : "mp4"}`
